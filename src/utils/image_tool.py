...
def batch_create_thumbnail(size, photo_dir=settings.PHOTO_DIR, filter_name=''):
    for photo in os.listdir(photo_dir):
        ...
        thumbnail = PicSize().pic_resize(photo_file, size[0], size[1])
        thumbnail.save(get_thumbnail_uri(photo_file))


def batch_normalized_photo(size, photo_dir=settings.PHOTO_DIR, filter_name=''):
    for photo in os.listdir(photo_dir):
        if filter_name and not re.match(filter_name, photo):
            continue
        photo_file = os.path.join(photo_dir, photo)
        resized_pic = PicSize().pic_resize(photo_file, size[0], size[1])
        resized_pic.save(photo_file)